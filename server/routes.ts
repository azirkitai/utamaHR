...
export function registerRoutes(app: Express): Server {
      ...
      const templateData = {
        company: {
          ...
          logoHTML: companySettings?.logoUrl ? 
            `<img src="${companySettings.logoUrl}" class="company-logo" alt="Company Logo" style="width:60px;height:60px;object-fit:contain;display:block;border:none;" />` : 
            ""
        },
        employee: {
  ...
  async function generatePayslipPDFResponse(employeeId: string, documentId: string, res: any) {
      ...
      if (templateData.company.logoHTML && companySettings?.logoUrl) {
        ...
        if (logoBase64) {
          templateData.company.logoHTML = `<img src="${logoBase64}" class="company-logo" alt="Company Logo" style="width:60px;height:60px;object-fit:contain;display:block;border:none;" />`;
        }
      }
