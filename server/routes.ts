...
export function registerRoutes(app: Express): Server {
  ...
  // =================== SALARY CALCULATION SETTINGS ROUTES ===================
  // Get salary calculation settings (HRDF configuration)
  app.get("/api/salary-calculation-settings", authenticateToken, async (req, res) => {
    try {
      const { settingName = 'global' } = req.query;
      const settings = await storage.getSalaryCalculationSettings(settingName as string);
      res.json(settings);
    } catch (error) {
      console.error("Error fetching salary calculation settings:", error);
      res.status(500).json({ error: "Gagal mendapatkan tetapan pengiraan gaji" });
    }
  });

  // Create or update salary calculation settings (HRDF configuration)
  app.post("/api/salary-calculation-settings", authenticateToken, async (req, res) => {
    try {
      const currentUser = req.user!;
      
      // Only admin roles can modify salary calculation settings
      const adminRoles = ['Super Admin', 'Admin', 'HR Manager', 'Finance/Account'];
      if (!adminRoles.includes(currentUser.role)) {
        return res.status(403).json({ error: "Tidak dibenarkan mengubah tetapan pengiraan gaji" });
      }

      const { insertSalaryCalculationSettingSchema } = await import("@shared/schema");
      const validatedData = insertSalaryCalculationSettingSchema.parse(req.body);
      
      const settings = await storage.upsertSalaryCalculationSettings(
        validatedData.settingName || 'global', 
        validatedData
      );
      
      res.json(settings);
    } catch (error) {
      console.error("Error saving salary calculation settings:", error);
      res.status(500).json({ error: "Gagal menyimpan tetapan pengiraan gaji" });
    }
  });

  // Update salary calculation settings
  app.put("/api/salary-calculation-settings/:id", authenticateToken, async (req, res) => {
    try {
      const currentUser = req.user!;
      ...
      const { updateSalaryCalculationSettingSchema } = await import("@shared/schema");
      const validatedData = updateSalaryCalculationSettingSchema.parse(req.body);
      const updatedSettings = await storage.updateSalaryCalculationSettings(req.params.id, validatedData);
      
      if (!updatedSettings) {
        return res.status(404).json({ error: "Tetapan pengiraan gaji tidak dijumpai" });
      }
      
      res.json(updatedSettings);
    } catch (error) {
      console.error("Error updating salary calculation settings:", error);
      res.status(500).json({ error: "Gagal mengemas kini tetapan pengiraan gaji" });
    }
  });
